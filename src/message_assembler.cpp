...
MessageAssembler::AssembledMessage 
MessageAssembler::get_assembled_message(uint64_t message_id) {
    std::lock_guard<std::mutex> lock(mutex_);
...
void MessageAssembler::cleanup_old_messages(uint64_t max_age_ms) {
    std::lock_guard<std::mutex> lock(mutex_);
    
    auto now = std::chrono::steady_clock::now();
    auto it = assembly_map_.begin();
    
    while (it != assembly_map_.end()) {
